...
export default function HomePage() {
  ...
  return (
    <div style={{ background:"var(--bg)" }}>
      ...
      {/* ── WHO WE ARE ── */}
      <section style={{ padding:"clamp(48px,8vw,96px) clamp(20px,5vw,96px)", background:"var(--bg)", overflow:"hidden" }}>
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <div className="reveal">
            <div className="eyebrow-dark mb-4">Who We Are</div>
            <h2 className="font-display mb-5" style={{ fontSize:"clamp(28px,4.5vw,52px)", fontWeight:500, color:"var(--navy)", lineHeight:1.08 }}>
              A Legacy Built on<br/><em style={{ fontStyle:"italic", color:"var(--gold)" }}>Trust &amp; Innovation</em>
            </h2>
            <p className="mb-6" style={{ fontSize:"clamp(14px,1.5vw,15px)", lineHeight:1.85, color:"var(--text2)", fontWeight:300 }}>
            ...
            <div style={{ marginTop:"32px", display:"grid", gridTemplateColumns:"repeat(3, 1fr)", borderTop:"1px solid rgba(164,199,61,0.2)", paddingTop:"24px" }}>
              {[
                ...
                <div key={s.label} style={{ borderRight:i<2?"1px solid rgba(164,199,61,0.15)":"none", paddingRight:"clamp(8px,2vw,16px)", paddingLeft: i>0 ? "clamp(8px,2vw,16px)" : "0" }}>
                  <p className="font-display mb-1" style={{ fontSize:"clamp(24px,3vw,42px)", fontWeight:300, lineHeight:1, color:"var(--navy)" }}>
                    <Counter target={s.target} suffix={s.suffix}/>
                  </p>
                  <p style={{ fontSize:"clamp(8px,1vw,10px)", letterSpacing:"0.1em", textTransform:"uppercase", fontWeight:600, color:"var(--gold)" }}>{s.label}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="reveal-left hidden md:block" style={{ position:"relative", padding:"24px" }}>
      ...
      <section style={{ background:"#F0F7EE", padding:"clamp(48px,8vw,96px) clamp(20px,5vw,96px)", overflow:"hidden" }}>
        <div className="max-w-7xl mx-auto w-full">
          <div className="grid lg:grid-cols-5 gap-10 lg:gap-16 items-center">
            <div className="lg:col-span-2 reveal" style={{ position:"relative" }}>
              <div className="grid grid-cols-3 gap-2 md:gap-3" style={{ position:"relative", zIndex:1 }}>
                {[
                  ...
                  <div key={i} style={{ animationDelay:`${i*0.12}s` }}>
                    ...
                    <div style={{ padding:"8px 6px", background:"#F0F7EE", borderLeft:"1px solid var(--line)", borderRight:"1px solid var(--line)", borderBottom:"1px solid var(--line)", borderRadius:"0 0 8px 8px" }}>
                      <p style={{ fontSize:"clamp(8px,1.5vw,11px)", fontWeight:700, color:"var(--navy)", marginBottom:"2px", lineHeight:1.3 }}>{f.name}</p>
                      <p style={{ fontSize:"clamp(7px,1vw,9px)", letterSpacing:"0.12em", textTransform:"uppercase", color:"var(--gold)", fontWeight:700 }}>Founder</p>
                    </div>
                  </div>
                ))}
              </div>
            </div>
            <div className="lg:col-span-3 reveal d1" style={{ marginTop:"clamp(24px,4vw,0px)" }}>
              <div className="eyebrow-dark mb-4">Leadership</div>
              <h2 className="font-display" style={{ fontSize:"clamp(24px,3.5vw,44px)", fontWeight:500, color:"var(--navy)", lineHeight:1.1, marginBottom:"8px" }}>
                Message from<br/>
                <em style={{ fontStyle:"italic", color:"var(--gold)" }}>the Founders</em>
              </h2>
              <div style={{ width:"48px", height:"3px", background:"var(--gold)", marginBottom:"24px" }}/>
              <p style={{ fontSize:"clamp(13px,1.3vw,17px)", fontStyle:"italic", color:"var(--navy)", borderLeft:"3px solid var(--green)", paddingLeft:"16px", marginBottom:"20px", lineHeight:1.7, fontWeight:300 }}>
              ...
              <div className="space-y-4" style={{ fontSize:"clamp(14px,1.5vw,16px)", lineHeight:1.85, color:"var(--text2)", fontWeight:300 }}>
      ...
      <section style={{ background:"#F0F7EE", padding:"clamp(48px,8vw,96px) clamp(20px,5vw,96px)", overflow:"hidden" }}>
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-2 gap-8 items-end mb-12 reveal">
            <div>
              <div className="eyebrow-dark mb-4">Our Journey</div>
              <h2 className="font-display" style={{ fontSize:"clamp(28px,5vw,64px)", fontWeight:500, color:"var(--navy)", lineHeight:1.0, letterSpacing:"-0.02em" }}>
                A Legacy Built<br/>
                <em style={{ fontStyle:"italic", color:"var(--green)" }}>Year on Year</em>
              </h2>
            </div>
            <div>
              <p style={{ fontSize:"clamp(14px,1.5vw,16px)", lineHeight:1.85, color:"var(--text2)", fontWeight:300 }}>
          ...
          <div className="journey-scroll-track" style={{ display:"flex", flexDirection:"column", width:"max-content" }}>
            <div style={{ display:"flex", flexDirection:"row" }}>
              {[...milestones, ...milestones].map((m, i) => (
                <div key={i} style={{ width:"clamp(160px, 22vw, 320px)", flexShrink:0, padding:"0 clamp(16px,3vw,48px)", height:"180px" }}>
                  <div className="font-display" style={{ fontSize:"clamp(36px,5vw,80px)", fontWeight:700, color:"var(--green)", lineHeight:1, marginBottom:"12px", letterSpacing:"-0.02em" }}>{m.year}</div>
                  <p style={{ fontSize:"clamp(11px,1.1vw,16px)", color:"var(--navy)", fontWeight:400, lineHeight:1.4 }}>{m.desc}</p>
                </div>
              ))}
            </div>
            <div style={{ height:"24px" }}/>
            <div style={{ display:"flex", flexDirection:"row", alignItems:"center", position:"relative", height:"24px" }}>
              <div style={{ position:"absolute", top:"50%", left:0, right:0, height:"1px", background:"rgba(1,8,44,0.2)", transform:"translateY(-50%)", zIndex:0 }}/>
              {[...milestones, ...milestones].map((m, i) => (
                <div key={i} style={{ width:"clamp(160px, 22vw, 320px)", flexShrink:0, padding:"0 clamp(16px,3vw,48px)", display:"flex", alignItems:"center" }}>
                  <div style={{ width:"20px", height:"20px", borderRadius:"50%", background:"var(--green)", boxShadow:"0 0 0 5px rgba(164,199,61,0.2), 0 0 12px rgba(164,199,61,0.6)", zIndex:2, flexShrink:0 }}/>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── CSR SECTION ── */}
      <section style={{ background:"#f8fdf4", padding:"clamp(48px,8vw,96px) clamp(20px,5vw,96px)", position:"relative", overflow:"hidden" }}>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <div>
              ...
              <h2 className="font-display reveal d1" style={{ fontSize:"clamp(24px,3.5vw,46px)", fontWeight:500, color:"var(--navy)", lineHeight:1.1, marginBottom:"16px" }}>
                Committed to <em style={{ fontStyle:"italic", color:"var(--blue)" }}>People &amp; Community</em>
              </h2>
              <p className="reveal d2" style={{ fontSize:"clamp(14px,1.5vw,16px)", color:"var(--text2)", lineHeight:1.85, fontWeight:300, marginBottom:"24px" }}>
            ...
            <div className="reveal-left" style={{ marginTop:"clamp(24px,4vw,0px)" }}>
              <div style={{ background:"linear-gradient(135deg, var(--navy) 0%, #050e3a 100%)", padding:"clamp(24px,4vw,48px)", position:"relative", overflow:"hidden" }}>
                <div style={{ position:"absolute", top:0, left:0, right:0, height:"3px", background:"linear-gradient(90deg, var(--green) 0%, var(--gold) 100%)" }}/>
                <p style={{ fontSize:"clamp(36px,5vw,72px)", fontWeight:700, color:"var(--green)", lineHeight:1, marginBottom:"12px" }}>CSR</p>
                <p className="font-display" style={{ fontSize:"clamp(16px,2vw,26px)", fontWeight:500, color:"white", lineHeight:1.3, marginBottom:"16px" }}>
      ...
      <section style={{ background:"var(--navy)", padding:"clamp(48px,8vw,96px) clamp(20px,5vw,96px)", position:"relative", overflow:"hidden" }}>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div style={{ textAlign:"center", marginBottom:"clamp(40px,6vw,72px)" }} className="reveal">
            <div className="eyebrow mb-4" style={{ color:"rgba(255,255,255,0.5)" }}>Sustainability & HSE</div>
            <h2 className="font-display" style={{ fontSize:"clamp(28px,4vw,56px)", fontWeight:500, color:"white", lineHeight:1.1, marginBottom:"16px" }}>
              Building a <em style={{ fontStyle:"italic", color:"var(--green)" }}>Greener Tomorrow</em>
            </h2>
            <p style={{ fontSize:"clamp(14px,1.5vw,16px)", color:"rgba(255,255,255,0.55)", lineHeight:1.85, fontWeight:300, maxWidth:"680px", margin:"0 auto" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {[
              ...
              <div key={c.label} className="reveal" style={{ animationDelay:`${i*0.12}s`, padding:"clamp(24px,3vw,40px) clamp(20px,2.5vw,32px)", background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.08)", borderTop:`3px solid ${c.accent}`, backdropFilter:"blur(8px)" }}>
                <div style={{ fontSize:"36px", marginBottom:"16px" }}>{c.icon}</div>
                <h3 className="font-display" style={{ fontSize:"clamp(16px,1.5vw,20px)", fontWeight:700, color:"white", marginBottom:"12px" }}>{c.label}</h3>
      ...
      {/* ── NEWS ── */}
      <section style={{ background:"var(--bg)", padding:"clamp(48px,8vw,96px) clamp(20px,5vw,96px)" }}>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 reveal">
          <div>
            ...
            <h2 className="font-display" style={{ fontSize:"clamp(26px,4vw,48px)", fontWeight:500, color:"var(--navy)" }}>
          ...
          {newsItems.map((n,i)=>(
            <div key={i} className="news-card reveal" style={{ animationDelay:`${i*0.1}s` }}>
              ...
              <div className="p-5">
                ...
                <h3 className="font-display font-normal mb-3 leading-snug" style={{ fontSize:"clamp(15px,1.5vw,18px)", color:"var(--navy)" }}>{n.title}</h3>
                <p className="text-[13px] font-light leading-relaxed mb-4" style={{ color:"var(--text2)" }}>{n.excerpt}</p>
