...
// Standardized 3 cols: Year Incorporated | PSX Symbol | Core Business
...
export default function WhatWeDoStory({ industries }: { industries: Industry[] }) {
  ...
  if (!isDesktop) {
    return (
      <>
        <section style={{ background:"var(--bg2)", padding:"72px clamp(24px,6vw,64px) 24px" }}>
          <div className="eyebrow-dark mb-4">What We Do</div>
          <h2 className="font-display" style={{ fontSize:"clamp(28px,6vw,42px)", fontWeight:300, color:"var(--navy)", lineHeight:1.05, letterSpacing:"-0.01em" }}>
            Diverse Industries,<br/><em style={{ fontStyle:"italic", color:"var(--blue)" }}>Singular Excellence</em>
          </h2>
          <p className="mt-5" style={{ fontSize:"15px", lineHeight:1.75, color:"var(--text2)", fontWeight:300 }}>
            Four companies. One vision. Scroll through the story of the Ghani Global Group.
          </p>
        </section>
        {industries.map((ind, i) => (
          <section key={ind.id} style={{ background: i%2 === 0 ? "var(--bg2)" : "var(--bg)", padding:"48px clamp(24px,6vw,64px)", borderBottom:"1px solid var(--line)" }}>
            <img src={ind.img} alt={ind.title} style={{ width:"100%", aspectRatio:"4/3", objectFit:"cover", marginBottom:"24px" }}/>
            <div className="flex items-center gap-3 mb-3">
              <span style={{ flex:1, height:"1px", background:"var(--line2)" }}/>
              <span style={{ fontSize:"10px", letterSpacing:"0.22em", textTransform:"uppercase", fontWeight:700, color:"var(--green2)" }}>{ind.tab}</span>
            </div>
            <h3 className="font-display" style={{ fontSize:"26px", fontWeight:300, color:"var(--navy)", lineHeight:1.15, marginBottom:"14px" }}>{ind.title}</h3>
            <p style={{ fontSize:"15px", lineHeight:1.75, color:"var(--text2)", fontWeight:300, marginBottom:"22px" }}>{ind.desc}</p>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"8px", paddingTop:"18px", borderTop:"1px solid var(--line)", marginBottom:"22px" }}>
              {statsMap[ind.id]?.map(s => (
                <div key={s.lbl}>
                  <p className="font-display" style={{ fontSize:"18px", fontWeight:500, color:"var(--navy)", lineHeight:1, marginBottom:"5px" }}>{s.num}</p>
                  <p style={{ fontSize:"9px", letterSpacing:"0.18em", textTransform:"uppercase", fontWeight:600, color:"var(--text2)" }}>{s.lbl}</p>
                </div>
              ))}
            </div>
        ...
        <div style={{ padding:"32px clamp(32px,5vw,72px) 20px", background:"var(--bg2)", borderBottom:"1px solid var(--line)" }}>
          <div className="max-w-7xl mx-auto flex items-end justify-between gap-8">
            <div>
              ...
              <h2 className="font-display" style={{ fontSize:"clamp(24px,2.8vw,36px)", fontWeight:300, color:"var(--navy)", lineHeight:1.1, letterSpacing:"-0.01em" }}>
                Diverse Industries,{" "}
                ...
                >
                  <div className="story-slide-grid grid grid-cols-12 items-center">
                    <div className="col-span-6" style={{ position:"relative" }}>
                      <div className="story-img-wrap" style={{ position:"relative" }}>
                        ...
                        <div style={{ position:"absolute", inset:0, overflow:"hidden", pointerEvents:"none", zIndex:1 }}>
                          ...
                          <div style={{ position:"absolute", top:0, right:"10%", width:"1px", height:"35%", background:"linear-gradient(180deg, var(--gold) 0%, transparent 100%)", transform:"rotate(12deg)", transformOrigin:"top center", opacity:0.35 }}/>
                        </div>
                    ...
                    <div className="col-span-6">
                      ...
                      <h3 className="font-display story-heading-hover" style={{ fontSize:"clamp(20px,2vw,32px)", fontWeight:300, color:"var(--navy)", lineHeight:1.1, letterSpacing:"-0.01em", marginBottom:"18px", cursor:"default" }}>
                        {ind.title}
                      </h3>
